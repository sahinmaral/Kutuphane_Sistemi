...
namespace Kutuphane_Sistemi.UI
{
    public partial class Book_Writer : Form
    {
        ...
        private void btn_order_book_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if(control==0)
                {
                    ...
                    {
                        con.Open();
                        SqlCommand penalty_update = new SqlCommand("UPDATE SET student penalty_date=@penalty_date where st_tr_id=@st_tr_id", con);
        ...
        {
            txt_student_n.Text = Student_Query.name;
            txt_student_s.Text = Student_Query.surname;
            s_id = Student_Query.id_no;
