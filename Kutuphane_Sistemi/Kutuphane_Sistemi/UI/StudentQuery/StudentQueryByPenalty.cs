...
namespace Kutuphane_Sistemi.UI.StudentQuery
{
    public partial class StudentQueryByPenalty : Form
    {
        ...
        private void BtnScanStudent_Click(object sender, EventArgs e)
        {
            ...
            DbConnection.Open();
            SqlCommand sqlCommand = new SqlCommand(Join.StudentJoin + " s.penalty_date IS NOT NULL", DbConnection);
        ...
        private void BtnScanStudent2_Click(object sender, EventArgs e)
        {
            SqlConnection DbConnection = new SqlConnection(Shortcon.Address);

            DbConnection.Open();
            SqlCommand sqlCommand = new SqlCommand(Join.StudentJoin + " s.penalty_date IS NULL", DbConnection);
